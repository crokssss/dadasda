...
public class DialogScreenListener {
    ...
    public void onScreenInit(ScreenEvent.Init.Post event) {
        ...
        if (!matchesKeyword(event.getScreen())) {
            return;
        }
        // Новое окно диалога (или CustomNPCs пересоздал тот же экран) — начинаем следить заново.
        this.activeScreen = event.getScreen();
        this.lastCandidate = "";
        this.stableCount = 0;
    }
    ...
    public void onTick(TickEvent.ClientTickEvent event) {
        if (event.phase != TickEvent.Phase.END || activeScreen == null) return;
        if (!Config.ENABLED.get()) return;

        Screen screen = activeScreen;
        if (Minecraft.getInstance().screen != screen) {
            // Окно исчезло без события Closing (бывает) — сбрасываем состояние.
            activeScreen = null;
            lastCandidate = "";
            stableCount = 0;
            return;
        ...
        if (text.equals(lastCandidate)) {
            stableCount++;
        } else {
            lastCandidate = text;
            stableCount = 0;
            return; // текст ещё дописывается/меняется — ждём следующего тика
        }

        // Текст не менялся stableTicks тиков подряд — считаем его окончательным.
        if (stableCount != Config.STABLE_TICKS.get()) return;
    ...
    private static String asText(Object value) {
        ...
        if (value instanceof Map<?, ?> m) return joinText(orderedValues(m));
    ...
    /**
     * HashMap не гарантирует порядок перебора. Если ключи сравнимы (например, номер строки
     * диалога), сортируем по ним, иначе получим предложения в случайном порядке ("набор слов").
     */
    @SuppressWarnings({"unchecked", "rawtypes"})
    private static Collection<?> orderedValues(Map<?, ?> m) {
        try {
            ...
            if (comparable) {
                entries.sort((a, b) -> ((Comparable) a.getKey()).compareTo(b.getKey()));
            }
            List<Object> values = new ArrayList<>(entries.size());
            for (Map.Entry<?, ?> e : entries) values.add(e.getValue());
            return values;
        } catch (Throwable t) {
            return m.values();
        }
