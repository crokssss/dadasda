...
public final class Config {
    ...
    public static final ForgeConfigSpec.IntValue STABLE_TICKS;
    ...
    static {
        ...
        DEBUG = b.comment("Печатать в свой чат все найденные поля и тексты экрана.")
                .define("debug", false);
        STABLE_TICKS = b.comment(
                        "Сколько тиков (1 тик = 1/20 сек) текст должен быть неизменным, прежде чем",
                        "он считается дописанным и озвучивается. Увеличь, если текст в GUI печатается",
                        "по буквам/словам медленно и озвучка всё ещё срабатывает раньше конца.")
                .defineInRange("stableTicks", 4, 1, 100);
        b.pop();
